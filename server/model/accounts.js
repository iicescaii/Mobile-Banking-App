...
const checkMobileExist = async (mobile) => {
  return new Promise((resolve, reject) => {
    pool.query(
      "SELECT * FROM user_profile WHERE mobile_number = ?",
      [mobile],
      (err, results) => {
        if (err) {
...
// Check for dob existence
const checkDobExist = async (dob) => {
  return new Promise((resolve, reject) => {
    pool.query(
      "SELECT * FROM user_profile WHERE date_of_birth = ?",
      [dob],
      (err, results) => {
        if (err) {
        ...
        console.log("dob check results:", results);
        resolve(results.length > 0);
      }
    );
  });
